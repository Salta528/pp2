...
ay = 18
fruit_timer = time.time()  # Таймер для отслеживания времени с момента появления фрукта
...
while True:
    ...
    if time.time() - fruit_timer > 5:  # Если прошло 5 секунд, создаем новый фрукт
        a = random.choice(possible_possition)
        ax = a[0]
        ay = a[1]
        fruit_timer = time.time()  # Обновляем таймер для нового фрукта
